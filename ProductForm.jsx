...
export default function ProductForm({ initial, categories, onSubmit, submitting }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <Field
          label="SKU (unique product code)"
          name="sku"
          value={form.sku}
          onChange={handleChange}
          required
          disabled={!!initial}
          placeholder="e.g. SC-LEH-001"
        />
        {initial ? (
          <p className="text-xs text-inkmuted mt-1.5">SKU cannot be changed after a product is created.</p>
        ) : (
          <p className="text-xs text-inkmuted mt-1.5">
            Must be unique. Products can share the same name as long as the SKU is different.
          </p>
        )}
      </div>
...
function Field({ label, ...props }) {
  return (
    <label className="block">
      ...
      <input
        {...props}
        className="w-full border border-rose-200 rounded-md px-3.5 py-2.5 text-sm outline-none focus:border-rose-500 disabled:bg-blush/40 disabled:text-inkmuted disabled:cursor-not-allowed"
      />
    </label>
  );
}
